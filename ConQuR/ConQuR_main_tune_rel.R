...
# relative abundance: ConQuR_rel (no information about libsize)
# count: ConQuR, ConQuR_libsize 

### ConQuR_rel main function 

ConQuR_rel <- function(tax_tab, batchid, covariates,
                       batch_ref,
  ...
  if (simple_match == T){
    ...
    tax_new = foreach (ll=1:ncol(tax_tab), .combine=cbind) %do%{
      y = as.numeric( tax_tab[, ll] )
      ConQuR_each_rel(y=y, X=X, X_span=X_span, X_correct=X_correct, X_span_correct=X_span_correct, batchid=batchid, batch_ref=batch_ref,
...
### Tune ConQuR_rel over variations

Tune_ConQuR_rel <- function(tax_tab, batchid, covariates,
                            batch_ref_pool,
                            logistic_lasso_pool,
                            quantile_type_pool,
                            simple_match_pool,
  ...
  for (current_ref in 1:length(batch_ref_pool)){
    ...
    for (current_cutoff in 1:length(cut_list)){
      ...
      for (current_method in 1:nrow(method1)){
        tab_list[[1+current_method]] = ConQuR_rel(tax_tab=tab, batchid=batchid, covariates=covariates,
      ...
      if (method2 == T){
        tab_list[[1+nrow(method1)+1]] = ConQuR_rel(tax_tab=tab, batchid=batchid, covariates=covariates,
                                                   batch_ref=as.character(batch_ref_pool[current_ref]),
                                                   logistic_lasso=F,
    ...
    if (!length( cut_list_remaining ) == 0){
      ...
      tax_new_remaining = ConQuR_rel(tax_tab=tab, batchid=batchid, covariates=covariates,
                                     batch_ref=as.character(batch_ref_pool[current_ref]),
                                     logistic_lasso=F,
                                     quantile_type="standard",
                                     simple_match=F,
