...
########## ConQuR for relative abundance, no library size information: help functions ##########
# no rounding and use kappa (pseudo library size) when dealing with relative abundance only #


### fast computation of zero-inflated conditional quantile function

# fast computation (not by exact tau), consider jittered relative abundance 
proposed.method.fast.rel <- function(logistic, xl, quantile, qmat, n, delta, taus, logistic_lasso, interplt, kappa){
  ...
  for (ii in 1:nrow(xl)){
    ...
    if (length(Taus.s[[ii]]) > 0 & Taus.s[[ii]][1] < 1){
      ...
      if (interplt == T){
        temp_qf = c( rep(0, P1[[ii]]), ceiling( fit[1]*P2[[ii]]/kappa-1 )*kappa, ceiling( fit[-1]/kappa-1 )*kappa )
      } else{
        temp_qf = c( rep(0, P1[[ii]]), ceiling( fit/kappa-1 )*kappa )
      }
      quant[ii, ] = temp_qf
...
simple_QQ_rel <- function(y, batchid, batch_ref, taus){
...
ConQuR_each_rel <- function(y, X, X_span, X_correct, X_span_correct, batchid, batch_ref, 
  ...
  tryCatch({
    ...
    # jitter relative abundance 
    kappa = min(y_sub)
    y_sub = y_sub + runif(length(y_sub), min=0, max=kappa)
  ...
  ziqr.fast = proposed.method.fast.rel(logistic=logistic_fit, xl=X_logistic,
                                       quantile=quant_coef, qmat=X_quantile,
                                       n=length(y),
                                       delta=delta, taus=taus, logistic_lasso=logistic_lasso, interplt=interplt,
                                       kappa=kappa)
  
  ziqr.fast.correct = proposed.method.fast.rel(logistic=logistic_fit, xl=X_logistic_correct,
