...
proposed.method.fast.libsize <- function(logistic, xl, quantile, qmat, n, libsize, delta, taus, logistic_lasso, interplt){
  ...
  for (ii in 1:nrow(xl)){
    ...
    if (length(Taus.s[[ii]]) > 0 & Taus.s[[ii]][1] < 1){
      ...
      ###### libsize specific ######
      if (interplt == T){
        temp_qf = c( rep(0, P1[[ii]]), ceiling( fit[1]*P2[[ii]]*libsize[ii]-1 ), ceiling( fit[-1]*libsize[ii]-1 ) ) / libsize[ii]
      } else{
        temp_qf = c( rep(0, P1[[ii]]), ceiling( fit*libsize[ii]-1 ) ) / libsize[ii]
      }
...
ConQuR_each_libsize <- function(y, X, X_span, X_correct, X_span_correct, batchid, batch_ref,
  ...
  if ( sum( table(sy) <= logistic_thres ) | length(table(sy)) == 1 | logistic_lasso == F ){
    ...
    libsize_standardized = scale(libsize, center=T, scale=T)
    X_logistic = cbind(X, libsize_standardized)
    X_logistic_correct = cbind(X_correct, libsize_standardized)
