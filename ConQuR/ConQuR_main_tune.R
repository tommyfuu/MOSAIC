...
ConQuR <- function(tax_tab, batchid, covariates,
  ...
  if (simple_match == T){
    ...
    tax_new = foreach (ll=1:ncol(tax_tab), .combine=cbind) %do%{
      y = as.numeric( tax_tab[, ll] )
      ConQuR_each(y=y, X=X, X_span=X_span, X_correct=X_correct, X_span_correct=X_span_correct, batchid=batchid, batch_ref=batch_ref,
